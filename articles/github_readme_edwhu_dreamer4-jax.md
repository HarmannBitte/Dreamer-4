<!-- source: https://github.com/edwhu/dreamer4-jax
     fetched: 2026-09-20T13:37:32Z -->

Note

See [open-dreamer](https://github.com/next-state/open-dreamer) for the upgraded version of the model that works on Minecraft!
...
- **Core library (`dreamer/`)**  - `models.py` -- Space-time axial attention, causal tokenizer, interactive dynamics model, agent / reward / value heads.
...
- **Training & evaluation scripts (`scripts/`)**  - `train_tokenizer.py` -- Trains the causal tokenizer (masked autoencoder over video).
...
- **Docs & logs**  - `docs/` -- Figures, videos, and notes from development (e.g., reconstructions, imagination rollouts).
...
Ground Truth, Masked Input, Reconstructions
...
Dynamics model training curves.
...
Reward / BC / Dynamics model losses.
The policy is trained with RL on imagination rollouts from the dynamics model.

RL training curves. The returns increase and the policy stays in the center.
