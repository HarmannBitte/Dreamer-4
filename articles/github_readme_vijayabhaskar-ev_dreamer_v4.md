<!-- source: https://github.com/vijayabhaskar-ev/dreamer_v4
     fetched: 2026-09-20T13:37:30Z -->
...
A PyTorch implementation of **DreamerV4** (Hafner, Yan & Lillicrap, DeepMind, 2025 — [arXiv:2509.24527](https://arxiv.org/abs/2509.24527)): a model-based agent that learns by *imagining* trajectories inside a learned world model. All three phases are implemented and run end-to-end — tokenizer → flow-matching world model → behavior-cloned agent → imagination RL — and evaluated **closed-loop in the real environment**, not just inside imagination.
  ...
  *Two runs of the **identical** pipeline. Top: a policy whose frozen reward model accumulates a belief of **574 reward** on an episode where the ball never lands in the cup (true catch rate 0.126). Bottom: a different Phase-2 draw — belief tracks reality (0.704). Both episodes are re-renders of **stored, released evaluation episodes**, verified bit-for-bit against `evaluation/tmlr-*/episodes.csv` (`analysis/render_hero.py`).*
...
- **Imagination compresses what reality separates.** Final*imagined* returns of the six runs span a narrow 74–78 band while their*real* catch rates span 0.366–0.460 — even after training, the world model cannot reliably tell you which policy is the strong one. Closed-loop evaluation is not optional.
  ...
  *Left: the real episode (480px re-render of the same physics; the model itself sees 128px). Right: the frozen reward model's cumulative predicted reward vs. what the environment actually paid. Top — exploiting run, board 29: belief ends at **+574**, reality at **0**. Bottom — healthy run, board 0: belief tracks reality to **920**. Board outcomes match the released per-episode logs exactly; on the exploiting run, 50/50 re-run boards reproduce their stored (return, caught) pairs.*
...
`mean ≈ argmax` to the decimal ⇒ the per-state conditionals are ~unimodal, so "deploy the mean, not the mode" **cannot** help. The collapse is the **deterministic-map / OOD-drift** effect — only sampling is robust to it, and the actor was *trained* on sampled actions, so sampling is the training-consistent deployment.
  ...
  *Left: the trained agent catching and holding the ball (stochastic policy). Right: the learned flow-matching world model — ground truth vs. autoregressive prediction, decoded to pixels.*
...
Yields a 240-episode, **mixed-quality** buffer (mean ~0.57 normalized return — see [Results](https://github.com#results--ball_in_cup_catch--real-env-closed-loop-n--50)). The data itself is not redistributed; only the conversion script is.
...
```
MIT — see [`LICENSE`](https://github.com/vijayabhaskar-ev/dreamer_v4/blob/main/LICENSE).