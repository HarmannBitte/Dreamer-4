...
SESSION_NAME="${SESSION_NAME:-wm_daf75k_mon8_physical_cuda2}"
RUN_NAME="${RUN_NAME:-waymo_direct_action_flow_generateall75k_mon8_physical_h15_b5_10k_20260911}"
...
echo "===== $(date -Is) mon8_physical training CUDA=$CUDA_DEVICE target=$MAX_STEPS ====="
"$PYTHON" "$TRAIN" --checkpoint "$CHECKPOINT" --train_data "$DATA_ROOT/train" --val_data "$DATA_ROOT/val" --output_dir "$OUT" --max_steps "$MAX_STEPS" --physical "${resume[@]}"
for b in 1 5 15; do
...
echo "===== $(date -Is) mon8_physical training/evaluation complete ====="
