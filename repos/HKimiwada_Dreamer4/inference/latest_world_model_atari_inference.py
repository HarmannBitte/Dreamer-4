...
class AtariInferenceConfig:
    ...
    wm_ckpt_path = Path("checkpoints/world_model/latest_atari/best_wm.pt")
...
def load_atari_models(cfg):
    ...
    builder_state = torch.load(
        "checkpoints/world_model/latest_atari/best_builder.pt",
        map_location="cpu",
    )
    builder.load_state_dict({k.replace("module.", ""): v for k, v in builder_state.items()})
...
def main():
    ...
    save_path = cfg.output_dir / "feburary_latest_atari_dream_latest.mp4"
